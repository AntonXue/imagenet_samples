...
def sample_and_copy_images(num_samples, dest_folder, imagenet_root="/data/imagenet/val"):
    """
    Sample images from each ImageNet class and copy them to a destination folder.
    
    Args:
        num_samples (int): Number of samples to take from each class
        dest_folder (str): Destination folder path to copy images to
    """
    
    # Create destination folder if it doesn't exist
    os.makedirs(dest_folder, exist_ok=True)
    
    for wordnet_id, label in wordnetid_to_label.items():
        # Get all images for this class
        source_folder = os.path.join(imagenet_root, wordnet_id)
        all_images = glob.glob(os.path.join(source_folder, "*.JPEG"))
        ...
        for i, img_path in enumerate(selected_images):
            new_name = f"{wordnet_id}_{label}_{i}.JPEG"
